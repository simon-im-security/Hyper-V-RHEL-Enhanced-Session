...
if [ "$(id -u)" -ne 0 ]; then 
    echo "Error: This script requires root privileges. Please rerun as root or using sudo."
    exit 1
fi
...
if [ "$major_version" -lt 9 ]; then
    echo "Error: Unsupported OS version. This script is designed for RHEL 9 and newer."
    exit 1
fi
...
echo "Configuration complete. Please reboot the system to apply the new settings."
...
echo "1. In Hyper-V, open your VM settings, go to Management > Integration Services, and enable 'Guest services'."
echo "2. Run the following PowerShell command as Administrator on the host to set the Enhanced Session Transport Type to HVSocket:"
echo "   Set-VM <VM_NAME> -EnhancedSessionTransportType HVSocket"
